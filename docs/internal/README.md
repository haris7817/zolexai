...
| [`character-replacement-full-length.md`](./character-replacement-full-length.md) | 🟡 **Character Replacement over the whole source, 6 Sep 2026.** Why the 10 s limit existed, what the frozen graph allows (24 fps loader, one reference picture that is frame 0), the windowed chain seeded with each window's last frame, the 2-minute total ceiling and its cost arithmetic, tests, GPU seam checks pending, rollback. |
